// 调换3_中位移和旋转的顺序， 先旋转，后位移: ： 箱子绕屏幕旋转
/*
* 记住，矩阵乘法是反向应用的。
因此，这次首先将平移应用于容器，将其定位在屏幕的右下角。
在平移之后，旋转被应用于已经平移的容器。

旋转变换也被称为基变换，当我们深入研究线性代数时会发现这一点。
由于我们正在改变容器的基，接下来的结果平移将基于新的基向量对容器进行平移。
一旦向量稍微旋转，垂直平移也会随之稍微偏移。
例如，如果先应用旋转，它们会围绕旋转原点（0,0,0）解析，但由于容器先被平移，
其旋转原点不再是（0,0,0），这使得它看起来像是围绕场景的原点做圆周运动。
*/
...
const char* vertexCodePath = "./src/chapter1/5_transform/2_shader.vertex";
const char* fragmentCodePath = "./src/chapter1/5_transform/2_shader.fragment";
